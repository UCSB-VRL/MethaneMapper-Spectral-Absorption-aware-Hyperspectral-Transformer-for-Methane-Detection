	...
	for _file in all_files:
		...
		if len(contours) > 0:
			...
			for _contour in contours:
				if (len(_contour) < 8) : continue #plume too small
				sqz_contour = _contour.squeeze(axis=1) # (width, height) OR (y, x)
				#bbox : (top, left, bottom, right) OR (x.min, ymin, x.max, y.max)
				plume_bbox.append([int(sqz_contour[:, 1].min()), int(sqz_contour[:, 0].min()), 
									int(sqz_contour[:, 1].max()), int(sqz_contour[:, 0].max())])
				plume_mask.append(sqz_contour.tolist())
				plume_cat.append(1)
			annot_template = {"patch_name": patch_name,
							  "patch_id": patch_id,
							  "image_id": file_name,
