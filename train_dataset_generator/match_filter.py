...
def segMatchedFilterRect(b_img_data, segmentation, target, num_sensors,
                         contours):
    ...
    print(
        f"Num clusters:{num_classes}, Rows:{rows}, Cols:{cols}, img_shape:{b_img_data.shape}"
    )
    ...
    for _count in range(num_sensors, all_snsr + num_sensors, num_sensors):
        ...
        for _cls in range(num_classes):
            ...
            b_img_data[
                0,
                0, :] = 0  #init values in case number of pixels in cluster is too low
            ...
            if (num_pxls == 0): continue
                ...
                try:
                    b_mean_cov_obj = algo.calc_stats(clustr_pxls,
                                                     mask=None,
                                                     index=None)
...
def segmentation_match_filter(b_img_data, segmentation, target, pxl_batch):
    ...
    for i in range(num_classes):
        segmentation_mask[i] = ~np.ma.make_mask(segmentation - 1 - i)
    print(
        f"Segmentation mask size: {segmentation_mask.shape}, type: {segmentation_mask.dtype}"
    )
    ...
    for i in range(num_classes):
        print(
            f"Calculating gausian stats, mean, cov of background for cluster {i}"
        )

        if args.segmentation_mf == 'column_wise':
            # Get all pixels from specific segmentation bin in columnwise order
            clustr_pxls = b_img_data.transpose()[:, segmentation_mask[i].
                                                 transpose()].transpose()
        ...
        clustr_pxls = clustr_pxls.reshape(clustr_pxls.shape[0], 1,
                                          clustr_pxls.shape[1])
        curr_idx = 0
        while (curr_idx < num_pxls):
            idx_range = min(pxl_batch,
                            num_pxls - curr_idx)  # Get indexible range
            print(
                f"	Computing stats for cluster {i} indices {curr_idx} to {curr_idx+idx_range}"
            )
            b_mean_cov_obj = algo.calc_stats(clustr_pxls[curr_idx:curr_idx +
                                                         idx_range, :, :],
                                             mask=None,
                                             index=None)
            print(f"	b_mean_cov_obj -> type: {type(b_mean_cov_obj)} ")
            print(
                f"	clustr_mf_output: {clustr_mf_output[curr_idx:curr_idx+idx_range].shape}"
            )

            # Progressively replace clustr_pxls with matched filter outputs
            print(f"	clustr_pxls: {clustr_pxls.shape}")
            a = matched_filter(
                clustr_pxls[curr_idx:curr_idx + idx_range, :, :], target,
                b_mean_cov_obj)
            print(
        ...
        print(
            f"	Assigning matched filter output of cluster {i} to correct pixel locations\n"
        )
...
def match_filter(b_img_data, target, num_columns):
    ...
    for _cols in range(0, cols, num_columns):
        ...
        b_mean_cov_obj = algo.calc_stats(b_img_data[:, _cols:col_range, :],
                                         mask=None,
                                         index=None)
