...
class DataLoader:
                ...
                if "glt.hdr" in _file:
                    ...
                    if not os.path.exists(os.path.join(root, _img_filename)):
                        ...
                        self.file_paths.append(
                            os.path.join(root, _img_filename))
    ...
    def load_data(self, file_path):
        ...
        self.row_size, self.col_size, self.channel = spy.envi.open(
            hdr_path).shape
        ...
        big_img_data = img_data_obj.read_subregion((0, self.row_size),
                                                   (0, self.col_size))
...
def main():
    ...
    t_mean = np.loadtxt(os.path.join(t_sig_path,
                                     os.listdir(t_sig_path)[0]))[:, -1]
    print(t_mean.shape)

    Parallel(n_jobs=1)(
        delayed(computeMF)(file_path, data_loader, index, locs, tiler,
                           pxl_batch_size, num_sensors, t_mean)
        for file_path in data_loader.file_paths)


def computeMF(file_path, data_loader, index, locs, tiler, pxl_batch_size,
              num_sensors, t_mean):
    ...
    big_img_data = data_loader.load_data(
        file_path)[:, :, 6:]  #removing corruputed channels
    ...
    if args.segmentation_mf:
        mf_output = segmentation_match_filter(big_img_data, segmentation,
                                              target, pxl_batch_size,
                                              all_contours)
    elif args.sensor_noisefree_mf:
        mf_output = segMatchedFilterRect(big_img_data, segmentation, target,
                                         num_sensors, all_contours)
    else:
    ...
    tiler.dataTiling(big_img_data[:, :, -90:],
                     filename,
                     dir_path=f'{ROOT}/rdata_tiles')
