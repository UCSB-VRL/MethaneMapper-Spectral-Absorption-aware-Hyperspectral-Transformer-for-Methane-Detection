...
from .segmentation import MMsegm, PostProcessPanoptic, PostProcessSegm, dice_loss, sigmoid_focal_loss
...
class MethaneMapper(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, aux_loss=False):
        ...
        # self.query_proj = nn.Conv2d(backbone.num_channels, hidden_dim, kernel_size=1)
    ...
    def forward(self, samples: NestedTensor, samples1: NestedTensor, samples2: NestedTensor):
        ...
        features, pos, mf_out, _, _ = self.backbone(samples.tensors, samples1.tensors, samples2.tensors)
        src, mask = features[-1].decompose()
        mf_query, mf_mask = mf_out[-1].decompose()
        assert mask is not None
        hs = self.transformer(self.input_proj(src), mask, self.query_embed.weight, pos[-1], mf_query)[0]
...
def build(args):
    ...
    if args.dataset_file == "hyper_spec":
        ...
        num_classes = 2
    elif args.dataset_file == "hyper_seg":
        num_classes = 2
